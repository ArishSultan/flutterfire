...
On iOS, macOS & Web (coming soon), before FCM payloads can be received on your device, you must first ask the users permission. Android applications are not required
to request permission.

The `firebase_messaging` package provides a simple API for requesting permission via the [`requestPermission`](!cloud_messaging.FirebaseMessaging.requestPermission) method.
...
To listen to messages whilst your application is in the foreground, listen to the [`onMessage`](!cloud_messaging.FirebaseMessaging.onMessage) stream.
...
be handled via the [`onBackgroundMessage`](!cloud_messaging.FirebaseMessaging.onBackgroundMessage) handler. When received, an
...
You should not rely data only messages to be delivered, they should only be used to
